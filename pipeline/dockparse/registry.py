    ...
    for tab in ("Science", "Yachts"):
        ...
        for row in wb[tab].iter_rows():
            ...
            for cell, v in cells:
                ...
                what = ("registry LOA/draft" if cur is not None else "REGISTRY_ORPHAN_SPECS") if (loa or draft) \
                    else "registry contact/notes (not imported)" if cur is not None else "registry header / stray text"
                mark(tab, cell, what)
    return out
