...
import {
  ...
  type BookingView, type ImportIssue, type ImportRun, type ISODate, type Project, type ProjectOrigin,
  type Settings, type Vessel, type Violation,
...
interface MockImport { run: ImportRun; issues: ImportIssue[]; staged: StagedRow[] }
...
function createProjectWorld(name: string, start: ProjectOrigin, asOfDate?: ISODate | null): MockProject {
  ...
  if (start === "sample") {
    ...
    return { meta, asOfDate: s.asOfDate, berths: s.berths, vessels: s.vessels, bookings: s.bookings, imports: [] };
  }
...
function fakeImport(p: MockProject, file: File | null, planTo?: string): MockImport {
  ...
  const issues: ImportIssue[] = [
    mkIssue("NO_BERTH", "error", yr, "K41", "This row sits on an unlabeled overflow line, so its berth is unknown.",
      { berthLabel: null, occupantType: "vessel", title: "R/V Golden Compass", vesselLengthFt: 124, startDate: d(10), endDate: d(13), classifiedBy: "regex" }),
    mkIssue("NO_BERTH", "error", yr, "S43", "This row sits on an unlabeled overflow line, so its berth is unknown.",
      { berthLabel: null, occupantType: "vessel", title: "S/V Far Horizon", vesselLengthFt: null, startDate: d(20), endDate: d(21), classifiedBy: "regex" }),
    mkIssue("OVERLAP", "error", yr, "H12", `${berths[0]?.name ?? "North Pier West"} is already held on those days by an earlier row.`,
      { berthLabel: berths[0]?.name ?? null, occupantType: "vessel", title: "F/V Long Drift", vesselLengthFt: 60, startDate: d(3), endDate: d(4), classifiedBy: "regex" }),
    mkIssue("VESSEL_TOO_LONG", "error", yr, "P18", "R/V High Drift (120′) is 30′ too long for South Float East (90′).",
      { berthLabel: "South Float East", occupantType: "vessel", title: "R/V High Drift", vesselLengthFt: 120, startDate: d(30), endDate: d(33), classifiedBy: "regex" }),
  ...
  const run: ImportRun = {
    ...
    counts: { sheets: format === "template" ? 3 : 23, cells: 9214, berths: 0, vessels: vesselNames.length, bookings: staged.length, outsideWindow: 1812, issues: issues.length },
    issueCounts: { error: 0, warning: 0, info: 0 },
  };
  recount({ run, issues, staged });
  return { run, issues, staged };
}
